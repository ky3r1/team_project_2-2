...
void SceneTutorial::Render()
{
	...
	// 2Dスプライト描画
	{
		...
		if (game_timer == 0)
		{
			...
			ui[7]->render(graphics.GetDeviceContext(), 200, 600, 1500, 750, 1, 1, 1, 0.7f, 0, 0, 0, 1920, 1080);
			ui[7]->end(graphics.GetDeviceContext());
			ui[2]->begin(graphics.GetDeviceContext(), 0);
			ui[2]->render(graphics.GetDeviceContext(), 500, 750, 283, 67, 1, 1, 1, 1, 0, 0, 0, 283, 67);
		...
		if (game_timer == 1)
		{
			...
			ui[1]->render(graphics.GetDeviceContext(), 850, 700, 300, 220, 1, 1, 1, 1, 0, 0, 0, 480, 360);
			ui[1]->end(graphics.GetDeviceContext());
			ui[3]->begin(graphics.GetDeviceContext(), 0);
			ui[3]->render(graphics.GetDeviceContext(), 500, 750, 375, 75, 1, 1, 1, 1, 0, 0, 0, 375, 75);
			ui[3]->end(graphics.GetDeviceContext());
			ui[4]->begin(graphics.GetDeviceContext(), 0);
			ui[4]->render(graphics.GetDeviceContext(), 500, 810, 416, 47, 1, 1, 1, 1, 0, 0, 0, 416, 47);
		...
		if (game_timer == 2)
		{
			...
			ui[0]->render(graphics.GetDeviceContext(), 850, 700, 300, 220, 1, 1, 1, 1, 0, 0, 0, 480, 360);
			ui[0]->end(graphics.GetDeviceContext());
			ui[5]->begin(graphics.GetDeviceContext(), 0);
			ui[5]->render(graphics.GetDeviceContext(), 500, 800, 300, 50, 1, 1, 1, 1, 0, 0, 0, 272, 46);
			ui[5]->end(graphics.GetDeviceContext());
			ui[10]->begin(graphics.GetDeviceContext(), 0);
			ui[10]->render(graphics.GetDeviceContext(), 500, 830, 300, 50, 1, 1, 1, 1, 0, 0, 0, 446, 68);
		...
		if (game_timer == 3)
		{
			...
			ui[9]->render(graphics.GetDeviceContext(), 500, 800, 300, 50, 1, 1, 1, 1, 0, 0, 0, 321, 46);
		...
		if (clear_check == true)
		{
			...
			ui[6]->render(graphics.GetDeviceContext(), 750, 700, 258, 258, 1, 1, 1, 1, 0, 0, 0, 258, 258);
			ui[6]->end(graphics.GetDeviceContext());
			ui[8]->begin(graphics.GetDeviceContext(), 0);
			ui[8]->render(graphics.GetDeviceContext(), 1100, 800, 200, 69, 1, 1, 1, 1, 0, 0, 0, 450, 69);
